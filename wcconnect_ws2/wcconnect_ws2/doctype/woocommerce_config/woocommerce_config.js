...
frappe.provide("wcconnect_ws2.woocommerce_config");

frappe.ui.form.on("WooCommerce Config", {
    onload: function(frm, dt, dn){
        frappe.call({
            method:"wcconnect_ws2.wcconnect_ws2.doctype.woocommerce_config.woocommerce_config.get_series",
            callback:function(r){
    ...
    refresh: function(frm){
        ...
        if(!frm.doc.__islocal && frm.doc.enable_woocommerce === 1){
            ...
            frm.add_custom_button(__('Sync WooCommerce'), function() {
                frappe.call({
                    method:"wcconnect_ws2.api.sync_woocommerce",
                })
            }).addClass("btn-primary");
            
            frm.add_custom_button(__("Sync WooCommerce IDs to ERP"), function(){
                frappe.call({
                    method:"wcconnect_ws2.api.sync_woocommerce_ids",
                })
            })
        }
        ...
        frappe.call({
            method: "wcconnect_ws2.api.get_log_status",
            callback: function(r) {
                if(r.message){
